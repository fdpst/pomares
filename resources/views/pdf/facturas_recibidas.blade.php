...
<body>
        ...
        <tr>
            ...
            <td style="width:100%">
                <table  style="width:100%">
                    <tr>
                        <td colspan="">
                        <div style="color: #546e7a; !important; font-weight:bold">REGISTRO DE I.V.A. SOPORTADO<div>
    ...
    <table class="tabla" style="width:100%; margin-top:80px">
        ...
        @endif
            <td>{{$index}}</td>
            <td>{{\Carbon\Carbon::parse($recibo->factura?->fecha)->format('d/m/Y')}}</td>
            <td>{{$recibo?->factura?->proveedor?->cuentaContable?->numero??'000000000'}}</td><!--Insertar Numero de Cuenta-->
            <td>{{$recibo?->factura?->proveedor?->nombre}}</td>
            <td>{{$recibo?->factura?->proveedor?->cif}}</td>
            <td>{{$recibo?->factura?->nro_factura}}</td>
            <td>ORD</td>
            <td>{{number_format( $recibo->cantidad*$recibo->precio,2,','.'')}}</td>
            <td>{{number_format( $recibo->iva,2,','.'')}}</td>
            <td>{{number_format( ($recibo->iva/100)*($recibo->cantidad*$recibo->precio),2,','.'')}}</td>
            <td></td>
            <td></td>
            <td></td>
        ...
        @php
            $index++;
            if(!isset($ivas[number_format( $recibo->iva,2,','.'')])){
                $ivas[number_format( $recibo->iva,2,','.'')] = ['base'=>0,'iva'=>0];
            }
            $ivas[number_format( $recibo->iva,2,','.'')]['iva'] +=($recibo->iva/100)*($recibo->cantidad*$recibo->precio);
            $ivas[number_format( $recibo->iva,2,','.'')]['base'] +=$recibo->cantidad*$recibo->precio;

            $base += $recibo->cantidad*$recibo->precio;
            $imp_iva += ($recibo->iva/100)*($recibo->cantidad*$recibo->precio);
            $total += $recibo->total;
        @endphp
